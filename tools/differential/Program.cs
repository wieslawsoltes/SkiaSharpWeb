...
SKMatrix Matrix(int i) {
    float sx = Next(), kx = Next(.2f), tx = Next(10), ky = Next(.2f), sy = Next(), ty = Next(10);
    float px = i % 3 == 0 ? Next(.0001f) : 0, py = i % 3 == 0 ? Next(.0001f) : 0;
    if (i % 64 == 0) sx = sy = kx = ky = px = py = 0;
    return new SKMatrix(sx, kx, tx, ky, sy, ty, px, py, 1);
}
var matrices = new List<object>();
for (int i = 0; i < 512; i++) {
    SKMatrix a = Matrix(i), b = Matrix(i+1); var point = new SKPoint(Next(10), Next(10));
...
for (int i=0;i<64;i++) {
    ...
    float cx=Next(2),cy=Next(2),radius=Math.Abs(Next(2))+2;
