...
for(const path of entries.keys()) {
  ...
  if(!/^(?:dist\/(?:lib|vendor|package|licenses)\/|dist\/(?:README|COMPATIBILITY)\.md$|docs\/(?:PACKAGING|RELEASING|BROWSER_TEXT|STARTUP)\.md$|blazor\/(?:README|INTEGRATION)\.md$|(?:package\.json|README(?:\.web)?\.md|CHANGELOG\.md|COMPATIBILITY\.md|LICENSE)$)/.test(path))throw new Error(`Unapproved package entry: ${path}`);
