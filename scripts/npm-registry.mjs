...
async function verify(tarball, distTag) {
  ...
  try {
    ...
    await mkdir(fresh);
    await writeFile(join(fresh, 'package.json'), JSON.stringify({ name: 'skiasharpweb-public-registry-consumer', version: '1.0.0', private: true, type: 'module' }));
    ...
    await writeFile(join(fresh, 'consumer.mjs'), await readFile(join(project, 'tests/packaging/consumer.mjs')));
    const smoke = spawnSync(process.execPath, [join(fresh, 'consumer.mjs')], { cwd: fresh, stdio: 'inherit', timeout: 60_000 });
    assert.equal(smoke.status, 0, 'Fresh npm installation must load bundled WASM and render PNG/PDF through ESM and CommonJS');
    ...
    await summary(`Published and verified **${pkg.name}@${pkg.version}** on npm (${distTag}).\n\nRelease and registry tarballs have identical SHA512 integrity. Public download, provenance metadata, installed ESM/CommonJS/TypeScript consumers, native WASM drawing, PNG/PDF encoding, runtime asset CLI, and fresh npm installation checks passed.\n\n[View npm package](https://www.npmjs.com/package/${pkg.name}/v/${pkg.version})`);
  } finally {
